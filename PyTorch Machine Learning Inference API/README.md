...
```bash
PyTorch Machine Learning Inference API$ sam build
```
...
```bash
PyTorch Machine Learning Inference API$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
PyTorch Machine Learning Inference API$ sam local start-api
PyTorch Machine Learning Inference API$ curl http://localhost:3000/classify_digit
```
...
```bash
PyTorch Machine Learning Inference API$ sam logs -n InferenceFunction --stack-name "pytorch-machine-learning-inference-api" --tail
```
...
```bash
sam delete --stack-name "pytorch-machine-learning-inference-api"
```
